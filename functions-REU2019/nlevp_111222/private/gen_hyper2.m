function [coeffs,fun,sol] = gen_hyper2(n,E,V)
%GEN_HYPER2    Hyperbolic QEP constructed from prescribed eigenpairs.
%  [COEFFS,FUN,SOL] = nlevp('gen_hyper2',N,S) generates an N-by-N
...
%  [COEFFS,FUN,SOL] = nlevp('gen_hyper2',E,S) generates Q(lambda)
%  with eigenvalues E, where E must be a vector of length 2*N of real
%  numbers ordered so that min(E(1:N)) > max(E(N+1:2*N)).
%  [COEFFS,FUN,SOL] = nlevp('gen_hyper2',E,V,S) is similar and produces
%  Q(lambda) with eigenvectors V, where V is an N-by-2*N matrix
%  satisfying V*[EYE(N) 0; 0 -EYE(N)]*V' = 0.
%  The optional parameter S (default: S = 0) is used in the function
%  call RANDN('state',S) to seed the random number generator (see HELP
%  RAND); if S < 0 then the newer syntax RNG(-S) is used.
...
if ~strcmpi(state,'noseed')
    if state >= 0
        randn('state',state)     % Set state for random numbers.
    else
