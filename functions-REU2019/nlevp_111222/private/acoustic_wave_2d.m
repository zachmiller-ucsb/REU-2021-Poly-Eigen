function [coeffs,fun] = acoustic_wave_2d(n,z)
%ACOUSTIC_WAVE_2D   Acoustic wave problem in 2 dimensions.
%  [COEFFS,FUN] = nlevp('acoustic_wave_2d',N,Z) constructs a quadratic
%  matrix polynomial lambda^2*M + lambda*D + K that arises from
...
%  form 2*pi*i*Z^(-1)*C, where C is a low rank real symmetric matrix and
...
h = 1/n1;
D = spdiags(ones(n1,1)*[-1,4,-1],-1:1,n1,n1);D(n1,n1) = 2;
T = spdiags(ones(n1-1,2),[-1,1],n1-1,n1-1);
S = speye(n1); S(n1,n1) = 0.5;
