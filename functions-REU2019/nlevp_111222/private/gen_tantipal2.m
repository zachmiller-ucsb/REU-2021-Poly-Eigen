function [coeffs,fun] = gen_tantipal2(n,T)
%GEN_TANTIPAL2 T-anti-palindromic QEP with eigenvalues on the unit circle.
%  [COEFFS,FUN] = nlevp('gen_tantipal2',N,S) generates an N-by-N
...
%  calls RAND('twister',S), RANDN('state',S) to seed the random number
%  generator (see HELP RAND); if S < 0 then the newer syntax RNG(-S) is used.
...
if nargin < 1, n = 16; T = []; state = 0; end
if nargin == 1
   if length(n) > 1
      T = n; n = length(T);
   else
