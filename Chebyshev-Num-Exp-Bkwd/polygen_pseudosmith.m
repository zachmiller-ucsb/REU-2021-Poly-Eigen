function [evs, P] = polygen_pseudosmith(d, polysize, ...
    pkmean, pkwidth)

% Some excessive arguments for Chebyshev ???maybe???, since nodes are fixed. Modify to
% take in fewer arguments.
...
% polynomial, given the size, degree, and node range.
    ...
    evs = 2*rand(d * polysize, 1) - 1; % A random vector that gives the desired eigenvalues in [-1,1]
